"""
Build sentiment database for the THIRD batch of 25 S&P 500 stocks.
...
# Third batch of 25 S&P 500 stocks (diversified across sectors)
BATCH_3_STOCKS = [
    # Large Cap Tech & Comm Services (5)
    "CSCO", "QCOM", "AMD", "AMAT", "AVGO",
    # Financials (5)
    "USB", "PNC", "TFC", "AXP", "SPGI",
    # Healthcare (5)
    "CVS", "CI", "HUM", "ISRG", "VRTX",
    # Consumer Discretionary (5)
    "AMZN", "TSLA", "LOW", "TJX", "BKNG",
    # Energy & Utilities (5)
    "CVX", "COP", "SLB", "NEE", "DUK"
]

def main():
    """Build sentiment cache for batch 3 stocks - AUTO RUN."""
    
    print("=" * 80)
    print("SENTIMENT DATABASE BUILDER - BATCH 3 (AUTO-RUN)")
    print("=" * 80)
    print(f"\nConfiguration:")
    print(f"  Stocks: {len(BATCH_3_STOCKS)}")
    print(f"  Date range: {START_DATE_STR} to {END_DATE_STR}")
    print(f"\nStocks to process:")
    for i, symbol in enumerate(BATCH_3_STOCKS, 1):
        print(f"  {i:2d}. {symbol}")
    ...
    # Estimate total time
    total_dates = len(BATCH_3_STOCKS) * 252 * 5  # Approx 252 trading days/year * 5 years
    ...
    for i, symbol in enumerate(BATCH_3_STOCKS, 1):
        print(f"\n{'='*80}")
        print(f"Stock {i}/{len(BATCH_3_STOCKS)}: {symbol}")
        print(f"{'='*80}")
        ...
        remaining = (len(BATCH_3_STOCKS) - i) * avg_time_per_stock
        
        print(f"\nProgress: {i}/{len(BATCH_3_STOCKS)} stocks completed")
        ...
        temp_file = f"sentiment_batch3_progress.csv"
    ...
    print(f"\nResults:")
    print(f"  Successful: {len(successful)}/{len(BATCH_3_STOCKS)}")
    print(f"  Failed: {len(failed)}/{len(BATCH_3_STOCKS)}")
    ...
    output_file = f"sentiment_build_batch3_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
...
if __name__ == "__main__":
    try:
        ...
        print("\n\nINTERRUPTED BY USER")
        print("Partial results saved in sentiment_batch3_progress.csv")
