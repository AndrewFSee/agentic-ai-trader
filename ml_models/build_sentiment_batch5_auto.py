"""
Build sentiment database for the FIFTH batch of 25 S&P 500 stocks.
...
# Fifth batch of 25 S&P 500 stocks (diversified across sectors)
# Previous batches: 1=config stocks, 2=ORCL,ADBE... 3=CSCO,QCOM... 4=LMT,MMM...
BATCH_5_STOCKS = [
    # Communication Services & Media (5)
    "T", "VZ", "CMCSA", "NFLX", "CHTR",
    # Retail & Consumer (5)
    "TGT", "LOWE", "ROST", "DHI", "LEN",
    # Semiconductors & Hardware (5)
    "MU", "ADI", "LRCX", "KLAC", "MCHP",
    # Pharma & Medical Devices (5)
    "PFE", "BMY", "ANTM", "MDT", "ABT",
    # Diversified Industrials & Services (5)
    "WM", "RSG", "FDX", "UPS", "NSC"
]

def main():
    """Build sentiment cache for batch 5 stocks - AUTO RUN."""
    
    print("=" * 80)
    print("SENTIMENT DATABASE BUILDER - BATCH 5 (AUTO-RUN)")
    print("=" * 80)
    print(f"\nConfiguration:")
    print(f"  Stocks: {len(BATCH_5_STOCKS)}")
    print(f"  Date range: {START_DATE_STR} to {END_DATE_STR}")
    print(f"\nStocks to process:")
    for i, symbol in enumerate(BATCH_5_STOCKS, 1):
        print(f"  {i:2d}. {symbol}")
    ...
    # Estimate total time
    total_dates = len(BATCH_5_STOCKS) * 252 * 5  # Approx 252 trading days/year * 5 years
    ...
    for i, symbol in enumerate(BATCH_5_STOCKS, 1):
        print(f"\n{'='*80}")
        print(f"Stock {i}/{len(BATCH_5_STOCKS)}: {symbol}")
        print(f"{'='*80}")
        ...
        remaining = (len(BATCH_5_STOCKS) - i) * avg_time_per_stock
        
        print(f"\nProgress: {i}/{len(BATCH_5_STOCKS)} stocks completed")
        ...
        temp_file = f"sentiment_batch5_progress.csv"
    ...
    print(f"\nResults:")
    print(f"  Successful: {len(successful)}/{len(BATCH_5_STOCKS)}")
    print(f"  Failed: {len(failed)}/{len(BATCH_5_STOCKS)}")
    ...
    output_file = f"sentiment_build_batch5_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
...
if __name__ == "__main__":
    try:
        ...
        print("\n\nINTERRUPTED BY USER")
        print("Partial results saved in sentiment_batch5_progress.csv")
