"""
Sentiment Database Builder - Batch 7 (Auto-Run)

This script collects 5 years of daily news sentiment for 25 S&P 500 stocks.
It will start automatically in 5 seconds.

Stocks in Batch 7: IBM, SHOP, PYPL, SQ, SO, D, AEP, HCA, DGX, SPG, WELL, O, 
                   TXN, ADP, PAYX, MCO, CME, ICE, CARR, APD, LIN, ECL, F, GM, DAL

NO REPEATS - Verified against batches 1-6 (150 stocks)
"""
...
# Seventh batch of 25 S&P 500 stocks (diversified across sectors)
# Verified NO REPEATS from batches 1-6
BATCH_7_STOCKS = [
    ...
    # Semiconductors & Tech Hardware (1)
    "TXN",
    ...
    # Industrials & Building (1)
    "CARR",
    
    # Chemicals & Industrial Gases (3)
    "APD", "LIN", "ECL",
...
def main():
    """Build sentiment cache for batch 7 stocks - AUTO RUN."""
    
    print("=" * 80)
    print("SENTIMENT DATABASE BUILDER - BATCH 7 (AUTO-RUN)")
    print("=" * 80)
    print(f"\nConfiguration:")
    print(f"  Stocks: {len(BATCH_7_STOCKS)}")
    print(f"  Date range: {START_DATE_STR} to {END_DATE_STR}")
    print(f"\nStocks to process:")
    for i, symbol in enumerate(BATCH_7_STOCKS, 1):
        print(f"  {i:2d}. {symbol}")
    ...
    # Estimate total time
    total_dates = len(BATCH_7_STOCKS) * 252 * 5  # Approx 252 trading days/year * 5 years
    ...
    for i, symbol in enumerate(BATCH_7_STOCKS, 1):
        print(f"\n{'='*80}")
        print(f"[{i}/{len(BATCH_7_STOCKS)}] Processing {symbol}")
        print(f"{'='*80}")
        ...
        print(f"  Remaining stocks: {len(BATCH_7_STOCKS) - i}")
    ...
    print(f"Average per stock: {total_elapsed/len(BATCH_7_STOCKS)/60:.1f} minutes")
    
    # Final cache stats
    final_stats = get_cache_stats()
    print(f"\nFinal cache: {final_stats['total_entries']} entries, {final_stats['unique_symbols']} symbols")
    print(f"Cache file: {final_stats['cache_file']}")
    print(f"{'='*80}\n")
